...
class Sts2Client:
    ...
    def get_current_decision(
        ...
        *,
        ...
        include_relevant_game_data: bool = False,
    ) -> dict[str, Any]:
        query = parse.urlencode(
            {
                "profile": profile,
                "include_raw_state": str(include_raw_state).lower(),
                "include_relevant_game_data": str(include_relevant_game_data).lower(),
            }
        )
        return self._request("GET", f"/v2/decision/current?{query}")
    ...
    def skip_rewards_and_proceed(self) -> dict[str, Any]:
        return self.execute_action(
            "skip_rewards_and_proceed",
            client_context={
                "source": "mcp",
                "tool_name": "skip_rewards_and_proceed",
            },
        )
