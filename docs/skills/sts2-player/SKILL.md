...
For full-run strategy, read `references/play-experience.md` before user-facing play sessions and apply its A10 functional gates throughout the run. It defines the default NOSL objective as defeating both Act 3 bosses, requires the run to track `frontload`, `defense`, `scaling`, `access`, and `matchup_gap`, and treats low-ascension wins only as weak synergy evidence rather than A10 deck templates. Read `references/encounter-notes.md` only for unfamiliar or decision-changing encounters, and still verify their current mechanics through live state/data. Read `references/run-retrospectives.md` for post-run review or strategy maintenance, not routine play. Maintain these references by replacing superseded claims, separating current mechanics from single-run observations, and extracting only generalizable lessons from raw logs. Never write run IDs, seeds, reconstructable per-turn action histories, or other log identifiers into skill references; keep those details in raw logs and store only anonymized decision patterns in the skill.
...
- Before an irreversible reward, shop, rest, boss-reward, or route choice, update the run's functional ledger: `frontload`, `defense`, `scaling`, `access`, and `matchup_gap`. A card with no named missing function is a skip by default after the early Act 1 survival package is established. By the end of Act 1, commit to one primary damage-scaling plan and one primary defense engine; do not open additional slow engines in Act 2 merely because their pieces are individually strong.
- Before elites and bosses, explicitly audit startup, defense, kill clock, and matchup coverage. For the Act 3 boss sequence, judge the first fight by the HP, potion, and startup resources expected to remain for the second fight, not by first-fight win probability alone.
...
- Verify low-HP self-damage and automatic-trigger order before committing to an irreversible power or line that depends on surviving it. `playable=true` means an action is legal, not nonlethal, and an enemy-intent-only end-turn preview does not prove that next-turn automatic HP loss is safe.
...
3. Branch by screen:
   ...
   - `REST`: compare the marginal completion-probability gain from healing, upgrading, removing, or other options. Do not use a fixed HP percentage; rest when healing crosses a real survival threshold and upgrade when strength/consistency is the larger failure mode. Treat reaching the Act 1 boss with no meaningful core upgrade, or repeatedly spending rest sites on healing, as a preparation warning that must change routing or deck expansion rather than becoming the default plan.
