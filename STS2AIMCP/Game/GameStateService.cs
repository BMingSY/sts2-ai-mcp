...
internal static class GameStateService
{
    ...
    public static AvailableActionsPayload BuildAvailableActionsPayload()
    {
        ...
        if (CanSkipRewardsAndProceed(currentScreen))
        {
            descriptors.Add(new ActionDescriptor
            {
                name = "skip_rewards_and_proceed",
                requires_target = false,
                requires_index = false
            });
        }
    ...
    {
        return currentScreen is NRewardsScreen;
    }

    public static bool CanSkipRewardsAndProceed(IScreenContext? currentScreen)
    {
        return currentScreen is NRewardsScreen &&
            IsProceedButtonUsable(GetRewardProceedButton(currentScreen));
    }

    public static bool CanClaimReward(IScreenContext? currentScreen)
    {
        return GetRewardButtons(currentScreen).Any(IsRewardButtonClaimable);
    }

    public static bool IsRewardButtonClaimable(NRewardButton button)
    {
        if (!button.IsEnabled)
        {
            return false;
        ...
        if (button.Reward is PotionReward)
        {
            return LocalContext.GetMe(RunManager.Instance.DebugOnlyGetState())?.HasOpenPotionSlots == true;
        }
    ...
    public static bool IsCombatPlayPhase()
    {
        try
        {
            var value = typeof(CombatManager).GetProperty("IsPlayPhase")?.GetValue(CombatManager.Instance);
    ...
    private static string[] BuildAvailableActionNames(IScreenContext? currentScreen, CombatState? combatState, RunState? runState)
    {
        ...
        if (CanSkipRewardsAndProceed(currentScreen))
        {
            names.Add("skip_rewards_and_proceed");
        }
    ...
    private static CombatPayload? BuildCombatPayload(CombatState? combatState)
    {
        ...
        return new CombatPayload
        {
            ...
            piles = BuildCombatPilesPayload(me.PlayerCombatState),
    ...
    private static CombatLethalRiskPayload[] BuildCombatLethalRiskPayloads(
        ...
        int unblockedDamage,
        int currentHp,
        bool endTurnWillKillPlayer)
    {
        if (!endTurnWillKillPlayer)
        {
            return Array.Empty<CombatLethalRiskPayload>();
        }

        return new[]
        {
            new CombatLethalRiskPayload
            {
                risk_id = "enemy_intent_damage",
                source = "enemy_intents",
                incoming_damage = incomingDamage,
                unblocked_damage = unblockedDamage,
                current_hp = currentHp,
                will_kill_player = true,
                description = "Ending the turn leaves enemy intent damage at or above current HP after block."
            }
        };
    }

    private static CombatPilesPayload BuildCombatPilesPayload(object? playerCombatState)
    {
        return new CombatPilesPayload
        {
            draw = BuildCombatPilePayload(
                "draw",
                ReadCombatPileCards(playerCombatState, "DrawPile", "DrawDeck")),
            discard = BuildCombatPilePayload(
                "discard",
                ReadCombatPileCards(playerCombatState, "DiscardPile")),
            exhaust = BuildCombatPilePayload(
                "exhaust",
                ReadCombatPileCards(playerCombatState, "ExhaustPile"))
        };
    }

    private static CombatPilePayload BuildCombatPilePayload(string pileName, IEnumerable<CardModel> cards)
    {
        var descriptors = cards
            .Select(BuildCombatPileCardDescriptor)
            .ToArray();
        var allStacks = descriptors
            .GroupBy(descriptor => descriptor.GroupKey, StringComparer.Ordinal)
            .Select(group =>
            {
                var first = group.First();
                var count = group.Count();
                return new CombatPileStackPayload
                {
                    ...
                    mods = first.mods,
                    count = count,
                    non_attack = first.non_attack,
                    block_card = first.block_card,
                    defensive_out = first.defensive_out,
                    line = FormatCardLine(
                        first.name,
                        first.upgraded,
                        count,
            ...
            .ThenBy(stack => stack.upgraded)
            .ThenBy(stack => stack.energy_cost)
            .ThenBy(stack => stack.star_cost)
            .ToArray();
        var stacks = allStacks.Take(MaxCombatPileStacks).ToArray();
        var countsByType = descriptors
            .GroupBy(descriptor => string.IsNullOrWhiteSpace(descriptor.card_type) ? "Unknown" : descriptor.card_type, StringComparer.OrdinalIgnoreCase)
            .ToDictionary(group => group.Key, group => group.Count(), StringComparer.OrdinalIgnoreCase);

        return new CombatPilePayload
        {
            pile = pileName,
            order = "hidden",
            count = descriptors.Length,
            stack_count = allStacks.Length,
            shown_stack_count = stacks.Length,
            truncated = allStacks.Length > stacks.Length,
            omitted_stack_count = Math.Max(0, allStacks.Length - stacks.Length),
            counts_by_type = countsByType,
            attack_count = descriptors.Count(descriptor => IsCardType(descriptor.card_type, "Attack")),
            skill_count = descriptors.Count(descriptor => IsCardType(descriptor.card_type, "Skill")),
            power_count = descriptors.Count(descriptor => IsCardType(descriptor.card_type, "Power")),
            status_count = descriptors.Count(descriptor => IsCardType(descriptor.card_type, "Status")),
            curse_count = descriptors.Count(descriptor => IsCardType(descriptor.card_type, "Curse")),
            non_attack_count = descriptors.Count(descriptor => descriptor.non_attack),
            block_card_count = descriptors.Count(descriptor => descriptor.block_card),
            defensive_out_count = descriptors.Count(descriptor => descriptor.defensive_out),
            non_attack_defensive_out_count = descriptors.Count(descriptor => descriptor.non_attack && descriptor.defensive_out),
            skill_defensive_out_count = descriptors.Count(descriptor => IsCardType(descriptor.card_type, "Skill") && descriptor.defensive_out),
            stacks = stacks
        };
    }

    private static CombatPileCardDescriptor BuildCombatPileCardDescriptor(CardModel card)
    {
        ...
        var blockCard = IsBlockCard(card, rulesText, keywords, mods);
        var defensiveOut = blockCard || IsDefensiveUtilityCard(rulesText, keywords, mods);

        return new CombatPileCardDescriptor(
            card.Id.Entry,
            card.Title,
            card.IsUpgraded,
            cardType,
            ...
            keywords,
            mods,
            !IsCardType(cardType, "Attack"),
            blockCard,
            defensiveOut);
    }

    private static bool IsCardType(string? cardType, string expected)
    {
        return string.Equals(cardType, expected, StringComparison.OrdinalIgnoreCase);
    }

    private static bool IsBlockCard(CardModel card, string rulesText, string[] keywords, string[] mods)
    {
        if (SafeReadBool(() => card.GainsBlock))
        {
            return true;
        }

        return ContainsAnyCardTerm(rulesText, keywords, mods, "Block", "格挡", "护甲");
    }

    private static bool IsDefensiveUtilityCard(string rulesText, string[] keywords, string[] mods)
    {
        return ContainsAnyCardTerm(
            rulesText,
            keywords,
            mods,
            "Weak",
    ...
    private static bool ContainsAnyCardTerm(string text, string[] keywords, string[] mods, params string[] terms)
    {
        return terms.Any(term =>
            (!string.IsNullOrWhiteSpace(text) && text.Contains(term, StringComparison.OrdinalIgnoreCase)) ||
            keywords.Any(keyword => keyword.Contains(term, StringComparison.OrdinalIgnoreCase)) ||
            mods.Any(mod => mod.Contains(term, StringComparison.OrdinalIgnoreCase)));
    }
        ...
        HashSet<string> glossaryTerms)
    {
        ...
        return new
        {
            player = new
            {
                ...
                powers = combat.player.powers.Select(FormatPowerLine).ToArray(),
            ...
            enemies = combat.enemies.Select(enemy => new
            {
                ...
                block = enemy.block,
                intent = enemy.intent,
                powers = enemy.powers.Select(FormatPowerLine).ToArray(),
    ...
    {
        var keywords = card.keywords.Length > 0
            ? card.keywords
            : GetGlossaryMatches(card.rules_text, card.mods);
        CollectGlossaryTerms(glossaryTerms, card.rules_text, card.mods);
    ...
    private static bool HasCardKeyword(IEnumerable<string> values, params string[] keywords)
    {
    ...
    private static bool CanRemoveCard(CardModel card, string[] mods, string[] keywords)
    {
        if (HasCardKeyword(mods.Concat(keywords), "Eternal", "永恒"))
        {
            return false;
        }

        // Ascender's Bane is defined as Eternal in the data files. Keep this as a
        // conservative fallback if the runtime card model omits keyword metadata.
        if (string.Equals(card.Id.Entry, "ASCENDERS_BANE", StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }
    ...
    private static IEnumerable<string> ExtractModifierTokens(object? value)
    {
        ...
        if (!string.IsNullOrWhiteSpace(fallback) &&
            !string.Equals(fallback, valueType.FullName, StringComparison.Ordinal))
        {
    ...
    private static string[] GetGlossaryMatches(string text, params string[][] modifierGroups)
    {
        ...
        foreach (var (keyword, _) in AgentKeywordDefinitions)
        {
            ...
            {
                if (modifierGroup.Any(modifier => modifier.Contains(keyword, StringComparison.OrdinalIgnoreCase)))
                {
                    values.Add(keyword);
                }
            }
    ...
    private static readonly (string keyword, string definition)[] AgentKeywordDefinitions =
    {
        ...
        ("Strength", "Each Strength usually increases attack damage by 1."),
        ("Dexterity", "Each Dexterity usually increases Block gained by 1."),
        ("Vulnerable", "A Vulnerable unit takes increased attack damage."),
        ("Weak", "A Weak unit deals reduced attack damage."),
        ("Frail", "A Frail unit gains reduced Block."),
        ("Block", "Block is removed before HP when taking damage."),
        ("Exhaust", "Exhausted cards leave combat for the current fight."),
        ("Retain", "Retained cards stay in hand at end of turn."),
        ("Ethereal", "If this card is in your hand at end of turn, it is Exhausted."),
        ("Eternal", "Cannot be removed or transformed from your deck."),
        ("Unplayable", "Cannot be played directly.")
    };
    ...
    private static RewardPayload? BuildRewardPayload(IScreenContext? currentScreen)
    {
        if (currentScreen is NRewardsScreen)
        {
            ...
            return new RewardPayload
            {
                ...
                can_proceed = CanSkipRewardsAndProceed(currentScreen),
...
internal sealed class CombatPayload
{
    ...
    public int player_turn_number { get; init; }

    public string? player_turn_phase { get; init; }
    ...
    public int powers_played_this_turn { get; init; }
...
{
    public CombatPilePayload draw { get; init; } = new();

    public CombatPilePayload discard { get; init; } = new();

    public CombatPilePayload exhaust { get; init; } = new();
}

internal sealed class CombatPilePayload
{
    public string pile { get; init; } = string.Empty;

    public string order { get; init; } = "hidden";

    public int count { get; init; }

    public int stack_count { get; init; }

    public int shown_stack_count { get; init; }

    public bool truncated { get; init; }

    public int omitted_stack_count { get; init; }

    public Dictionary<string, int> counts_by_type { get; init; } = new(StringComparer.OrdinalIgnoreCase);

    public int attack_count { get; init; }

    public int skill_count { get; init; }

    public int power_count { get; init; }

    public int status_count { get; init; }

    public int curse_count { get; init; }

    public int non_attack_count { get; init; }

    public int block_card_count { get; init; }

    public int defensive_out_count { get; init; }

    public int non_attack_defensive_out_count { get; init; }

    public int skill_defensive_out_count { get; init; }

    public CombatPileStackPayload[] stacks { get; init; } = Array.Empty<CombatPileStackPayload>();
}

internal sealed class CombatPileStackPayload
{
    ...
    public bool non_attack { get; init; }

    public bool block_card { get; init; }

    public bool defensive_out { get; init; }
...
internal readonly record struct CombatPileCardDescriptor(
    string card_id,
    string name,
    bool upgraded,
    string card_type,
